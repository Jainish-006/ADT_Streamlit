...
if st.button("Update Product Name for Selected Product"):
    df2.loc[df2['PRODUCTNAME'] == selected_product, 'PRODUCTNAME'] = new_product_name
    st.success(f"Product name for '{selected_product}' updated successfully!")
...
update_sql = f"UPDATE ADT_PROJECT.SUPERSTORE.SUPERSTORE_US SET "
update_sql += ", ".join([f"{col} = '{val}'" for col, val in df2.iloc[0].items()])
#session.sql(update_sql)
